...
//
	...
	fctg.y = rotateRight(binning(crossCorrData,altBinningFactor),altBinningFactor >> 1);
	fctg.x = rctg.x;
	fctl.title = '';
	fctl.annotations[0].text = rctl.annotations[0].text;
	crossCorrData = rotateRight(crossCorrData,rotSamples);

	rpg.y = rotateRight(binning(ggPSD,altBinningFactor),altBinningFactor >> 1);
	...
	fpg.y = rotateRight(binning(xyPSD,altBinningFactor),altBinningFactor >> 1);
	fpg.x = rpg.x;
	fpl.title = '';
