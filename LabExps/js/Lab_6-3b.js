...
var choices = choicesPoi;
var lambda = choices[0];
...
function process_6_3b()
	...
	let n0 = lengthMax;
	let lambda0 = (Math.sqrt(1+4*autoCorrZero/n0)-1)/2; // estimate #1
	let lambda1 = Math.sqrt((psdZero-autoCorrZero)/(n0*(n0-1))); // estimate #2
	let lambda2 = -1 + (autoCorrZero + Math.sqrt(1+ 4*psdZero)*autoCorrZero)/(2*psdZero); // estimate #3
	console.log('results = ', n0, lambda, lambda0.toFixed(3), lambda1.toFixed(3), lambda2.toFixed(3));
...
function prepareLab_6_3b( )
	...
	// get random choice of probability lambda from choices
	lower = 0;
	upper = choices.length-1;
	lambda = choices[randomInteger(lower,upper)];
	// initialize data in complex format
	for (var i = 0; i < 2*lengthMax; i += 2)
		rData[i] = randomPoisson(lambda);
