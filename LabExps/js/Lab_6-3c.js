...
function process_6_3c()
	...
	let n0 = lengthMax;
	let param0 = 0, param1 = 0, param2 = 0;
	if (noiseType == 'Exponential')
		{
		let param0 = Math.sqrt(2*n0/autoCorrZero); // estimate #1
		let param1 = 1/Math.sqrt((psdZero-autoCorrZero)/(n0*(n0-1))); // estimate #2
		let param2 = Math.sqrt(4*psdZero-2*autoCorrZero)/autoCorrZero; // estimate #3
		console.log('Exponential \u03b1 = ', 
			1/param, param0.toFixed(3), param1.toFixed(3), param2.toFixed(3), n0);
		}
	else if (noiseType == 'Poisson')
		{
		let param0 = (Math.sqrt(1+4*autoCorrZero/n0)-1)/2; // estimate #1
		let param1 = Math.sqrt((psdZero-autoCorrZero)/(n0*(n0-1))); // estimate #2
		let param2 = -1 + (autoCorrZero + Math.sqrt(1+ 4*psdZero)*autoCorrZero)/(2*psdZero); // estimate #3
		console.log('Poisson \u03bb = ', 
			param, param0.toFixed(3), param1.toFixed(3), param2.toFixed(3), n0);
		}
	else throw ('Problem in prepareLab_6_3c');
...
function getData(target)
	...
	if (digiState === 'ANALOG')
		...
		if (noiseType == 'Exponential')
			{
			document.querySelector('#noiseLabel').value = 'Exponential';
			document.querySelector('#paramLabel').value = '\u03b1'; // alpha
			document.getElementById('expEqn').setAttribute('style',yesNormDisplay);
			document.getElementById('poiEqn').setAttribute('style',noDisplay);
			document.querySelector('#var1Label').value = 'ee';
			document.querySelector('#var2Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#var3Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#var4Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#tn').value = 'e(t)';
			}
		else
			{
			document.querySelector('#noiseLabel').value = 'Poisson';
			document.querySelector('#paramLabel').value = '\u03bb'; // lambda
			document.getElementById('expEqn').setAttribute('style',noDisplay);
			document.getElementById('poiEqn').setAttribute('style',yesNormDisplay);
			document.querySelector('#var1Label').value = 'PP';
			document.querySelector('#var2Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#var3Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#var4Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#tn').value = 'P(t)';
			}
		...
		if (noiseType == 'Exponential')
			...
			document.querySelector('#var3Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#var4Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#tn').value = 'e[n]';
			}
		else
			{
			document.querySelector('#noiseLabel').value = 'Poisson';
			...
			document.querySelector('#var3Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#var4Label').value = document.querySelector('#var1Label').value;
			document.querySelector('#tn').value = 'P[n]';
			}
...
//
function prepareLab_6_3c(choice)
	{
	noiseType = choice;
	...
	// get random choice of probability parameter from choices
	if (noiseType == 'Exponential')
		{
		choices = choicesExp;
		lower = 0;
		upper = choices.length-1;
		param = 1/choices[randomInteger(lower,upper)];
		for (var i = 0; i < 2*lengthMax; i += 2)
			rData[i] = randomExponential(param); // data in complex format
		}
	else if (noiseType == 'Poisson')
		{
		choices = choicesPoi;
		lower = 0;
		upper = choices.length-1;
		param = choices[randomInteger(lower,upper)];
		for (var i = 0; i < 2*lengthMax; i += 2)
			rData[i] = randomPoisson(param); // data in complex format
		}
	else throw ('Problem in prepareLab_6_3c');
