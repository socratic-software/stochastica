...
//
// formatting of strings changed to be compatible with HTML Monday, 9 March 2020
// variables used in the string are now provided by call routine Monday, 9 March 2020
//		for example:	regression(depName, indName, 'power', data, order, options)
//						depNameY = depName;
//						indepNameX = indName;
...
var methods = {
	linear: function (data, _order, options) {
		...
		let plusminA = (gradient >= 0) ? ' ' : ' –';
		let plusminB = (intercept >= 0) ? ' + ' : ' – ';
		return {
			r2: determinationCoefficient(data, results),
			equation: [gradient, intercept],
			points: results,
			string: plusminA+Math.abs(_round(gradient, options.precision))+' '+indepNameX+
				plusminB+Math.abs(_round(intercept, options.precision)),
		};
	},
	...
	linearthroughorigin: function (data, _order, options) {
		...
		let plusminA = (_round(gradient, options.precision) >= 0) ? ' ' : ' –';
		return {
			r2: determinationCoefficient(data, results),
			equation: [gradient],
			points: results,
	...
	exponential: function (data, _order, options) {
		...
		let plusminA = (_round(coeffA, options.precision) >= 0) ? ' ' : ' –';
		let plusminB = (_round(coeffB, options.precision) >= 0) ? ' ' : ' –';
		return {
			r2: determinationCoefficient(data, results),
			equation: [coeffA, coeffB],
			points: results,
			string: plusminA+Math.abs(_round(coeffA, options.precision))+
				' e<sup>'+plusminB+Math.abs(_round(coeffB, options.precision))+' '+indepNameX+'</sup>',
		};
	},
	...
	logarithmic: function (data, _order, options) {
		...
		return {
			...
			points: results,
			string: plusminA+Math.abs(_round(coeffA, options.precision))+
				plusminB+Math.abs(_round(coeffB, options.precision))+' ln(' + indepNameX+')',
		};
	},
	...
	power: function (data, _order, options) {
		...
		return {
			...
			points: results,
			string: plusminA+Math.abs(_round(coeffA, options.precision))+' '+
				indepNameX+'<sup>'+plusminB+Math.abs(_round(coeffB, options.precision))+'</sup>',
		};
	},
	...
	polynomial: function (data, order, options) {
		...
		for (i = equation.length - 1; i >= 0; i--) {
			if (i > 1) {
				string += _round(equation[i], options.precision)+' '+indepNameX + 
				'<sup> ' + i + '</sup> + ';
			} else if (i === 1) {
				string += _round(equation[i], options.precision)+' '+indepNameX + ' + ';
			} else {
...
var depNameY = null;
var indepNameX = null;
exports = function regression(depName, indName, method, data, order, options) {
	depNameY = depName;
	indepNameX = indName;
	var methodOptions = (
